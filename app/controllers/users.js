...
class UsersControl {
  async find(ctx) {
    ...
    const users = await User.find({ name: new RegExp(ctx.query.keyword) })
      .limit(perPage)
      .skip(page * perPage);
    ctx.body = {
      code: 200,
  ...
  async followTopics(ctx) {
    const item = await User.findById(ctx.state.user._id).select(
      "+followingTopics"
    );
    if (
      !item.followingTopics.map((id) => id.toString()).includes(ctx.params.id)
    ) {
  ...
  async unfollowTopics(ctx) {
    ...
    const index = item.followingTopics
      .map((id) => id.toString())
      .indexOf(ctx.params.id);
    if (index !== -1) {
  ...
  async likeAnswers(ctx, next) {
    ...
    if (
      !item.likingAnswers.map((id) => id.toString()).includes(ctx.params.id)
    ) {
  ...
  async unlikeAnswers(ctx) {
    ...
    const index = item.likingAnswers
      .map((id) => id.toString())
      .indexOf(ctx.params.id);
    if (index !== -1) {
  ...
  async dislikeAnswers(ctx, next) {
    ...
    if (
      !item.dislikingAnswers.map((id) => id.toString()).includes(ctx.params.id)
    ) {
  ...
  async undislikeAnswers(ctx) {
    ...
    const index = item.dislikingAnswers
      .map((id) => id.toString())
      .indexOf(ctx.params.id);
    if (index !== -1) {
  ...
  async collectAnswer(ctx) {
    ...
    if (
      !item.collectingAnswers.map((id) => id.toString()).includes(ctx.params.id)
    ) {
  ...
  async uncollectAnswer(ctx) {
    ...
    const index = item.collectingAnswers
      .map((id) => id.toString())
      .indexOf(ctx.params.id);
    if (index !== -1) {
